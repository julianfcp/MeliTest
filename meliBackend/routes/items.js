...
router.get("/query/:query", async (req, res) => {
    ...
    const result = await fetch(`${process.env.URL_MELI_API}/sites/MLA/search?q=${req.params.query}`)
...
router.get("/:id", async (req, res) => {
    ...
    const result = await fetch(`${process.env.URL_MELI_API}/items/${req.params.id}`)
    const json = await result.json();
